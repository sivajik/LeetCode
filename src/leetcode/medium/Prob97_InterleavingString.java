...
public class Prob97_InterleavingString {
	...
	static public boolean isInterleave(String s1, String s2, String s3) {
		...
		boolean[][] visited = new boolean[s1.length() + 1][s2.length() + 1]; // make sure its +1
		return dfs(s1, s2, s3, 0, 0, 0, visited);
	}

	static private boolean dfs(String s1, String s2, String s3, int i, int j, int k, boolean[][] visited) {
